...
-- Dados do produto 1
SELECT venda_id, quantidade, unidade, valor_unitario, data_venda
FROM vendas_itens2
WHERE produto_id = 1;

-- Valores pelos quais o produto 1 foi vendido
SELECT valor_unitario
FROM vendas_itens2
WHERE produto_id = 1
GROUP BY valor_unitario;

-- Valor total das vendas do produto 1
SELECT venda_id, (valor_unitario*quantidade) AS valor_total
FROM vendas_itens2
WHERE produto_id = 1;

-- Quantidade total vendida de cada produto
SELECT produto_id, SUM(quantidade) AS qtd_vendido
FROM vendas_itens2
GROUP BY produto_id
ORDER BY produto_id ASC;

-- Quantidade total vendida, valor total das vendas e media do valor unitario de cada produto
SELECT
    produto_id, SUM(quantidade) AS qtd_total,
    unidade,
    SUM(valor_unitario*quantidade) AS valor_total,
    ROUND((SUM(valor_unitario*quantidade)/SUM(quantidade)),3) AS media_valor_unitario
FROM vendas_itens2
GROUP BY produto_id, unidade
ORDER BY produto_id ASC;

-- Valor total de cada venda
SELECT venda_id, ROUND(SUM(quantidade*valor_unitario),2) AS valor_total, data_venda
FROM vendas_itens2
