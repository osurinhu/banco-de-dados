...
SELECT venda_id, produto_id, valor_unitario * 1.10 AS valor_venda
FROM vendas_itens
WHERE venda_id = 2001;
...
SELECT * FROM vendas_itens
WHERE valor_unitario < 20 OR valor_unitario > 200
ORDER BY valor_unitario;

SELECT * FROM vendas_itens
WHERE produto_id IN (1,2,3);

SELECT venda_id, SUM(valor_unitario) AS total_venda
FROM vendas_itens
GROUP BY venda_id
